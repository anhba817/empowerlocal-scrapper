...
class RobertsonCountySourceSpider(scrapy.Spider):
    name = 'RobertsonCountySource'
    download_delay = 1.0
    custom_settings = {
        'RETRY_TIMES': 10,
    }
    ...
    def parse(self, response):
        ...
        post_selectors = response.css('.tdb-block-inner').css('.tdb_module_loop')
        for post in post_selectors:
            article_url = post.css('.td-module-title').xpath('./a/@href').get()
