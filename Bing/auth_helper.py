...
def authenticate(authorization_data):
    ...
    authorization_data.account_id=[acc.Id for acc in accounts['AdvertiserAccount']]
    authorization_data.customer_id=[acc.ParentCustomerId for acc in accounts['AdvertiserAccount']]

def authenticate_with_oauth(authorization_data):

    authentication=OAuthDesktopMobileAuthCodeGrant(
        client_id=bing_auth['client_id'],
        env=ENVIRONMENT
    )
