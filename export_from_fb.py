...
TOKEN = 'EAAo0zJUVDzABAHPzhjGqpDYdn8WZBWCb2FiZAdXszXd7ZCpJx86SZBMQHoDwoDZBsbvJXVCkS39BNVRsmCUJNrkZABZCJyuIdbZAWTYUB4k9oW5ZAV2Rx7yVD6xj6xZAhAt0WQAHWJdZBDkMIN8vbNPXQFwaytQoWZB8m2rwZCcZAZBlUpqD7Tex2WiO7SVesUmOuDjZAXAZD'
SDK_VERSION = '3.0'
...
graph = facebook.GraphAPI(access_token=TOKEN, version=SDK_VERSION)
...
with open('results.csv', 'w') as myFile:
    ...
    for worksheet in sh.worksheets():
        ...
        for ind, post in enumerate(all_posts_in_month):
            print("    Processing post %s: %s" % (ind, post['message']))
            ...
            if "message" in post:
                ...
                if search:
                    ...
                    if 'location' in i.headers:
                        ...
                        urlpath = url
                    if WEB_SPIDERS[worksheet.title]["url"] in urlpath:
                        ...
                        if sponsor_post:
                            ...
                            div_tag = soup.find("div", class_=WEB_SPIDERS[worksheet.title]["class_name"])
                            if div_tag and div_tag.find('ul'):
                                ...
                                for tag in tags:
                                    ...
                                    if info:
                                        ...
                                        break
            csvWriter.writerow([WEB_SPIDERS[worksheet.title]["name"], customer, post_message, created_time, total_reach, total_impressions,engaged_users, clicks])
